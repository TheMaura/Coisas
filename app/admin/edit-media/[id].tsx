...
import { Legend, Media } from '@/types';
...
export default function EditMediaScreen() {
  ...
  const fetchMedia = async () => {
    try {
      ...
      if (data) {
        // Inferir tipo pela URL se a coluna type não existir
        const url = (data.url || '').toLowerCase();
        const inferredType = url.includes('.mp4') || url.includes('.mov') || url.includes('video')
          ? 'video'
          : 'image';
        ...
        setSelectedLegend(data.legend_id || '');
      }
    } catch (error: any) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Atualizar dados sem a coluna type (ela não existe no schema atual)
      ...
      if (error) throw error;
      Alert.alert('Sucesso', 'Mídia atualizada com sucesso!', [
        { text: 'OK', onPress: () => router.back() },
      ]);
    } catch (error: any) {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      >
        ...
        <Text style={styles.headerTitle}>Editar Mídia</Text>
      </LinearGradient>
