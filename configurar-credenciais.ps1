...
Write-Host "=== Configuração de Credenciais Android para EAS ===" -ForegroundColor Cyan
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "Você não está logado. Fazendo login..." -ForegroundColor Yellow
    eas login
    if ($LASTEXITCODE -ne 0) {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "Agora você pode fazer builds pelo site da Expo:" -ForegroundColor Cyan
